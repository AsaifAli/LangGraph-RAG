...
def _ensure_qdrant_payload_indexes(client: Any, collection_name: str) -> None:
    """Ensure payload indexes required by the tenant/document filters exist.

    Qdrant collection data may predate the current filter schema, so indexes
    are repaired lazily and idempotently before ingestion/retrieval.
    """
    from qdrant_client import models

    required = (
        ("meta_data.tenant_schema", models.PayloadSchemaType.KEYWORD),
        ("meta_data.document_id", models.PayloadSchemaType.KEYWORD),
        ("meta_data.thread_id", models.PayloadSchemaType.KEYWORD),
        ("meta_data.person_ids", models.PayloadSchemaType.KEYWORD),
        ("meta_data.org_ids", models.PayloadSchemaType.KEYWORD),
    )
    ...
    for field_name, field_schema in required:
        try:
            ...
            # Qdrant can report an already-existing index as an exception
            # depending on client/server versions. That state is safe to keep.
